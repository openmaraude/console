...
N'envoyez que les positions des taxis libres (statut `free`). Les taxis avec une télémétrie récente seront affichés
comme disponibles pour les moteurs de recherche.
...
#### Rayon de visibilité du taxi

<InfoBox>Nouveau !</InfoBox>

Le chauffeur peut ajuster le rayon dans lequel il est visible par un client en recherche de taxi.
En conformité avec le décret d'application, ce rayon est compris entre 150 et 500 mètres inclus.
Il est par défaut de la valeur maximum. Une fois changé, il est conservé quand le chauffeur se déconnecte.
