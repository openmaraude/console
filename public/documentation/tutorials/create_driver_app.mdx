...
Inversement ce tutoriel prend des raccourcis qui rendent l'application inapte à un déploiement en production. Elle prend en charge une partie du travail dévolu au _backend_ sur vos serveurs pour simplifier le code.

Nous n'abordons pas cet aspect, toujours dans un souci de simplicité, mais vous voudrez protéger d'une façon ou d'une autre cette application accessible par internet, même temporaire, elle utilise votre clé d'API de la plateforme de développement.
