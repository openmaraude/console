...
L'application client envoie un `POST /hails` avec notamment la position du client et le taxi demandé, et nous acquittons la demande après vérification
...
Si le chauffeur accepte, l'opérateur doit changer le statut du *hail* en `accepted_by_taxi`.

Si le chauffeur refuse, l'opérateur doit changer le statut du *hail* en `declined_by_taxi`.
...
Si le client confirme, l'opérateur de mobilité doit changer le statut du *hail* en `accepted_by_customer`.

Si le client annule, l'opérateur de mobilité doit changer le statut du *hail* en `declined_by_customer`.

Si le statut du *hail* ne change pas dans un délai de 30 secondes, l'API passe le *hail* en `timeout_customer` et le taxi est à nouveau `free`. Le client est informé de l'annulation de sa demande. Il peut solliciter un autre taxi alentour avec [le même identifiant de session](/documentation/topic_guides/session_id). Un client qui abuserait du système sans honorer ses demandes s'expose à une [période d'exclusion du service](/documentation/howto_guides/ban) ou autres conséquences qui sortent du cadre de cette documentation.
