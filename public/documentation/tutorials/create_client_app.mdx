...
$> npx nodemon
```
* `npx express-generator --git` permet de créer le squelette de l'application.
...
Remplacez ensuite le contenu du fichier `views/index.jade` par le formulaire suivant:
...
Créez le fichier `views/search_taxis.jade` avec le contenu suivant :
...
Ajoutez en haut du fichier `index/routes.js` l'import à axios :
...
* Le template `search_taxis.jade` retourné peut lister les taxis via la variable `taxis` (ou `error` en cas d'erreur de l'API).
...
* Si la demande est fructueuse, nous effectuons une redirection vers une autre page qui affiche les détails de la course et permet son statut.

##### Page d'erreur

Créez la page `views/order.jade` avec le contenu suivant :
...
* Les données relative à la demande de course (localisation du client, numéro de téléphone, identifiant et opérateur du taxi) ont été fournies par la page précédente et sont disponibles dans `req.body`.
...
Créez le fichier `views/hail_info.jade` avec le contenu suivant :
...
Effectuez une demande de course en prenant soit de mettre le statut du taxi à `free` depuis la console de développement, et d'envoyer la géolocalisation du taxi.
...
Vous remarquerez que le numéro de téléphone du taxi devient disponible dès lors que celui-ci a accepté la demande de course. Ainsi, en cas de besoin, le client peut prendre directement contact avec le chauffeur pour organiser la rencontr pour organiser la rencontre.
...
Ajoutez le code suivant à la fin du fichier de template `views/hail_info.jade`, sous code pour afficher le tableau :
...
Puis ajoutez la route suivant en bas du fichier `routes/index.js` :
...
Dans des conditions réelles, il sera évidemment nécessaire d'afficher une carte, d'avoir une gestion des comptes pour vos utilisateurs et de gérer les différentes erreurs que peuvent retourner l'API.
